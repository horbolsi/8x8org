...
# auto = local(llama.cpp)->ollama->cloud(openai/gemini/deepseek)->fallback
AI_PROVIDER=auto
...
# Tools permissions (VERY IMPORTANT)
EXEC_ENABLED=false
WRITE_ENABLED=false
...
def chat_llama_cpp(prompt: str) -> dict:
    """
    llama.cpp server (llama-server) default API:
...
def chat_deepseek(prompt: str) -> dict:
    ...
    # NOTE: DeepSeek API formats vary; this is a conservative best-effort.
    try:
...
def fallback_reply(prompt: str) -> dict:
    return _ok("fallback", "FlashTM8 is running ✅ No AI provider reachable right now.\nYou can still index/search/tools.\n\nTry indexing workspace and use search + exec tools.")
  ...
  <div class="grid">
    ...
    <div class="main">
      ...
      <div class="panel" id="tab-logs">
        <div class="panelTitle">📜 Logs</div>
        <div class="hint">This is the live console log path:</div>
        <pre class="out" id="logsOut">(Logs appear here after server runs)</pre>
      </div>
...
.sidebar{
  border-right:1px solid var(--border);padding:12px;background:rgba(18,26,39,.4);
}
.tabbtn{
  width:100%;padding:10px 12px;margin-bottom:8px;border-radius:12px;
...
.ghost{width:100%;padding:10px 12px;border-radius:12px;border:1px dashed var(--border);background:transparent;color:var(--muted);cursor:pointer}
.main{padding:16px}
.panel{display:none}
...
button{
  padding:10px 12px;border-radius:12px;border:1px solid rgba(124,92,255,.7);
  background:rgba(124,92,255,.15);color:var(--text);cursor:pointer;
}
.chatbox{
  height:52vh;overflow:auto;border:1px solid var(--border);border-radius:16px;
  padding:12px;background:rgba(18,26,39,.3);margin-bottom:10px;
}
...
.out{
  height:52vh;overflow:auto;border:1px solid var(--border);border-radius:16px;
  padding:12px;background:rgba(18,26,39,.3);white-space:pre-wrap;
}
...
chmod +x "$REPO/start_sovereign_console.sh"

echo ""
echo "==============================================="
echo "✅ Dashboard created at:"
echo "   $APP_DIR"
echo ""
echo "Start it with:"
