...
async function createApp(edgespark) {
  ...
  app.get("/api/bots", async (c) => {
    const allBots = await edgespark.db.select().from(db_schema_exports.bots);
  ...
  app.post("/api/bots", async (c) => {
    ...
    if (Array.isArray(body)) {
      for (const bot of body) {
        ...
        await edgespark.db.insert(db_schema_exports.bots).values({
          id,
          name,
          type,
          status,
          ...
          target: db_schema_exports.bots.id,
  ...
  app.get("/api/files", async (c) => {
    const allFiles = await edgespark.db.select().from(db_schema_exports.files).orderBy(desc(db_schema_exports.files.createdAt));
  ...
  app.post("/api/files", async (c) => {
    ...
    if (Array.isArray(body)) {
      for (const file of body) {
        ...
        await edgespark.db.insert(db_schema_exports.files).values({
          id,
          name,
          type,
          size,
          ...
          target: db_schema_exports.files.id,
  ...
  app.get("/api/logs", async (c) => {
    const allLogs = await edgespark.db.select().from(db_schema_exports.logs).orderBy(desc(db_schema_exports.logs.id)).limit(100);
  ...
  app.post("/api/logs", async (c) => {
    ...
    if (Array.isArray(body)) {
      for (const log of body) {
        ...
        await edgespark.db.insert(db_schema_exports.logs).values({ timestamp, level, message });
      }
    } else {
  ...
  app.get("/api/messages", async (c) => {
    const msgs = await edgespark.db.select().from(db_schema_exports.messages).orderBy(desc(db_schema_exports.messages.createdAt)).limit(50);
    return c.json(msgs.reverse());
  });
  app.post("/api/messages", async (c) => {
    const body = await c.req.json();
    const { user_id, username, content } = body;
    await edgespark.db.insert(db_schema_exports.messages).values({ userId: user_id, username, content });
    return c.json({ success: true });
  });
  app.get("/api/tasks", async (c) => {
    const allTasks = await edgespark.db.select().from(db_schema_exports.tasks);
  ...
  app.post("/api/tasks", async (c) => {
    ...
    if (Array.isArray(body)) {
      for (const task of body) {
        ...
        await edgespark.db.insert(db_schema_exports.tasks).values({
          id,
          userId: user_id,
          title,
          description,
          ...
          target: db_schema_exports.tasks.id,
  ...
  app.get("/api/profile/:userId", async (c) => {
    ...
    const profile = await edgespark.db.select().from(db_schema_exports.profiles).where(eq(db_schema_exports.profiles.userId, userId)).get();
    return c.json(profile || {});
  });
  app.post("/api/profile", async (c) => {
    const body = await c.req.json();
    const { user_id, ai_id, telegram_id, wallet_address } = body;
    await edgespark.db.insert(db_schema_exports.profiles).values({
      userId: user_id,
      aiId: ai_id,
      telegramId: telegram_id,
      walletAddress: wallet_address
    }).onConflictDoUpdate({
      target: db_schema_exports.profiles.userId,
