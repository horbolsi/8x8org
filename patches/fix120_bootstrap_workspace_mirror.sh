...
# Write the BIG bootstrap script into scripts/ (so you never have to paste it again)
cat > scripts/bootstrap_workspace_mirror.sh <<'EOF'
#!/usr/bin/env bash
set -Eeuo pipefail
...
chmod +x scripts/bootstrap_workspace_mirror.sh
echo "✅ Created scripts/bootstrap_workspace_mirror.sh (from patch)."
echo "Run it now:"
echo "  bash scripts/bootstrap_workspace_mirror.sh"
